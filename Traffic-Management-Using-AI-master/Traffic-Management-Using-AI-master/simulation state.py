...
# Modify these values for better vehicle generation and timing
simTime = 300       # simulation time
defaultRed = 150
defaultYellow = 5
defaultGreen = 20
defaultMinimum = 10
...
# Adjust vehicle speeds for smoother movement
speeds = {
    'car': 0.5,
    'bus': 0.3,
    'truck': 0.3,
...
def update_web_interface():
    try:
        ...
        requests.post('http://localhost:5000/update_data', json=data)
...
def open_dashboard():
    ...
    while retry_count < max_retries:
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"Server not ready (attempt {retry_count + 1}/{max_retries})")
    ...
    if retry_count == max_retries:
        print("Failed to connect to dashboard after multiple attempts")
...
class Main:
    ...
    # Start Flask server
    thread_server = threading.Thread(name="flaskServer", target=run_flask_server)
